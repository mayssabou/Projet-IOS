...
struct ProfileView: View {
    ...
    @State private var showAlert = false // Pour afficher l'alerte de suppression
    ...
    func fetchUserDetails() {
        guard let encodedEmail = email.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let url = URL(string: "https://eb72-197-3-6-252.ngrok-free.app/user/\(encodedEmail)") else {
    ...
    func deleteProfile() {
        // Procéder à la suppression du profil
        fetchUserByEmail(email: self.email) { userId in
            ...
            guard let url = URL(string: "https://eb72-197-3-6-252.ngrok-free.app/user/\(userId)") else {
                print("URL incorrecte")
                return
            }
            ...
            let task = URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                DispatchQueue.main.async {
                    ...
                    self.navigateTologin = true  // Active la navigation vers LoginView
                }
            }
            task.resume()
        }
    ...
    func fetchUserByEmail(email: String, completion: @escaping (String?) -> Void) {
        ...
        guard let url = URL(string: "https://eb72-197-3-6-252.ngrok-free.app/user/\(encodedEmail)") else {
            completion(nil)
            return
        }
...
struct editProfileView: View {
    ...
    func updateProfile() {
        ...
        guard let encodedEmail = email.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let url = URL(string: "https://eb72-197-3-6-252.ngrok-free.app/user/email/\(encodedEmail)") else {
            print("Invalid URL")
            return
        }
